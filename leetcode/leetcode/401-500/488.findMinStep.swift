...
func findMinStep(_ board: String, _ hand: String) -> Int {
    ...
    func clean(_ b: [Character]) -> [Character] {
        ...
        for c in b {
            while !st.isEmpty && c != st.last!.0 && 3 <= st.last!.1 {
                st.removeLast()
            }
    ...
    func dfs(_ board: [Character], _ hand: [Character]) -> Int {
        ...
        for j in hand.indices {
            if 0 < j && hand[j] == hand[j-1] {
                continue
            }
            
            for i in 0...board.count {
                if 0 < i && board[i-1] == hand[j] {
                    continue
                }
                ...
                if 0 < i && i < board.count && board[i-1] == board[i] && board[i] != hand[j] {
                    choose = true
                }
